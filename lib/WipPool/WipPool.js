...
Object.assign(
    WipPool.prototype,
    {
        ...
        getStats() {
            ...
            const demandLatency = now - demandHistory.time.max;
            const demandIntervalWithLatencyCorrection = demandLatency > demandInterval.avg
                ? new SignalAggregator(demandInterval, demandLatency)
                : demandInterval;
        ...
        scheduleForecast(demanded = 0) {
            ...
            const now = Date.now();
            const processMetrics = calculateProcessMetrics(stats, now, options);
