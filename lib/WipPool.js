...
Object.assign(
    WipPool.prototype,
    {
        ...
        getStats() {
            ...
            const inPrigressTimes = inProgress
                .map(({start}) => now - start);
            const prepares = new SignalAggregator(this.prepares.stats, ...inPrigressTimes);
        ...
        prepareWip() {
            ...
            wip
                .then(() => {
                    ...
                    const progresIndex = inProgress.indexOf(progress);
                    inProgress.splice(progresIndex, 1);
                });
            return wip;
        },
